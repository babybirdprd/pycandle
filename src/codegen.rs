...
/// Configuration extracted from GPT2 models
#[derive(Debug, Clone)]
struct GptConfig {
    vocab_size: usize,
...
impl Codegen {
    ...
    fn generate_struct(&self, model_name: &str) -> String {
        ...
        // Sort layers to maintain some order
    ...
    fn map_type(&self, py_type: &str) -> String {
        match py_type {
            ...
            // GPT2 types
            "GPT2Model" | "GPT2LMHeadModel" => "gpt2::GPTModel".to_string(),
            "GPT2Block" => "gpt2::TransformerBlock".to_string(),
            "GPT2Attention" => "gpt2::MultiHeadAttention".to_string(),
            "GPT2MLP" => "gpt2::FeedForward".to_string(),
    ...
    fn generate_impl(&self, model_name: &str) -> String {
        ...
        // Forward method (simplified sequence)
        code.push_str("    pub fn forward(&self, xs: &Tensor) -> Result<Tensor> {\n");
        code.push_str("        let mut x = xs.clone();\n");

        // We only generate a linear sequence if they are simple modules.
        // For complex models, the user will have to manually adjust this.
        for (layer_name, meta) in &layers {
            if !meta.is_leaf {
                continue;
            }
    ...
    fn generate_init(&self, layer_name: &str, meta: &LayerMeta) -> String {
        match meta.module_type.as_str() {
            ...
            "Conv1d" => {
                ...
                // Note: Candle conv1d config might need manual adjustments for dilation/groups
                format!(
            ...
            "GPT2Block" => {
                // GPT2Block needs config from parent; generate placeholder
                format!(
            ...
            "GPT2MLP" => {
                // GPT2MLP (FeedForward) needs config from parent
                format!(
